...
{-| Similar to `getZone`, but this task will overwrite the local zone with the
-}
getZoneOverriding : Dict String String -> Task Error ( String, Time.Zone )
getZoneOverriding overwrites =
    Time.getZoneName
        |> Task.andThen
            (\nameOrOffset ->
                case nameOrOffset of
                    Time.Name zoneName ->
                        case Dict.get zoneName overwrites of
                            Just zoneName2 ->
                                case Dict.get zoneName2 zones of
                                    Just zone ->
                                        Task.succeed ( zoneName2, zone () )

                                    Nothing ->
                                        Task.fail (NoDataForZoneName zoneName2)
